...
qp::send_awaitable::send_awaitable(qp *qp, local_mr *local_mr,
    ...
    : qp_(qp), local_mr_(*local_mr, 0, local_mr->length()), remote_mr_(), wc_(),
      opcode_(opcode) {}
qp::send_awaitable::send_awaitable(qp *qp, local_mr *local_mr,
                                   enum ibv_wr_opcode opcode,
                                   remote_mr const &remote_mr)
    : qp_(qp), local_mr_(*local_mr, 0, local_mr->length()),
      remote_mr_(remote_mr, 0, remote_mr.length()), opcode_(opcode) {}
qp::send_awaitable::send_awaitable(qp *qp, local_mr *local_mr,
                                   enum ibv_wr_opcode opcode,
                                   remote_mr const &remote_mr, uint32_t imm)
    : qp_(qp), local_mr_(*local_mr, 0, local_mr->length()),
      remote_mr_(remote_mr, 0, remote_mr.length()), imm_(imm), opcode_(opcode) {
}
qp::send_awaitable::send_awaitable(qp *qp, local_mr *local_mr,
                                   enum ibv_wr_opcode opcode,
                                   remote_mr const &remote_mr, uint64_t add)
    : qp_(qp), local_mr_(*local_mr, 0, local_mr->length()),
      remote_mr_(remote_mr, 0, remote_mr.length()), compare_add_(add),
      opcode_(opcode) {}
qp::send_awaitable::send_awaitable(qp *qp, local_mr *local_mr,
                                   enum ibv_wr_opcode opcode,
                                   ...
                                   uint64_t swap)
    : qp_(qp), local_mr_(*local_mr, 0, local_mr->length()),
      remote_mr_(remote_mr, 0, remote_mr.length()), compare_add_(compare),
      swap_(swap), opcode_(opcode) {}

qp::send_awaitable::send_awaitable(qp *qp, local_mr_view local_mr,
                                   enum ibv_wr_opcode opcode)
    : qp_(qp), local_mr_(local_mr), remote_mr_(), opcode_(opcode) {}
qp::send_awaitable::send_awaitable(qp *qp, local_mr_view local_mr,
                                   enum ibv_wr_opcode opcode,
                                   remote_mr_view remote_mr)
    : qp_(qp), local_mr_(local_mr), remote_mr_(remote_mr), opcode_(opcode) {}
qp::send_awaitable::send_awaitable(qp *qp, local_mr_view local_mr,
                                   enum ibv_wr_opcode opcode,
                                   remote_mr_view remote_mr, uint32_t imm)
    : qp_(qp), local_mr_(local_mr), remote_mr_(remote_mr), imm_(imm),
      opcode_(opcode) {}
qp::send_awaitable::send_awaitable(qp *qp, local_mr_view local_mr,
                                   enum ibv_wr_opcode opcode,
                                   remote_mr_view remote_mr, uint64_t add)
    : qp_(qp), local_mr_(local_mr), remote_mr_(remote_mr), compare_add_(add),
      opcode_(opcode) {}
qp::send_awaitable::send_awaitable(qp *qp, local_mr_view local_mr,
                                   enum ibv_wr_opcode opcode,
                                   remote_mr_view remote_mr, uint64_t compare,
                                   uint64_t swap)
    : qp_(qp), local_mr_(local_mr), remote_mr_(remote_mr),
      compare_add_(compare), swap_(swap), opcode_(opcode) {}

static inline struct ibv_sge fill_local_sge(local_mr_view mr) {
  struct ibv_sge sge = {};
...
qp::recv_awaitable::recv_awaitable(qp *qp, local_mr *local_mr)
    : qp_(qp), local_mr_(*local_mr, 0, local_mr->length()), wc_() {}

qp::recv_awaitable::recv_awaitable(qp *qp, local_mr_view local_mr)
